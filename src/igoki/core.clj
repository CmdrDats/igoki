...
(defn draw-index [^JFrame frame ^Graphics g {:keys [images]}]
  (let [gridsize (Math/ceil (Math/sqrt (count images)))
    ...
    (doseq [[c {:keys [mat title]}] (map-indexed vector images)]
      (if-let [image (if (pos? (.width mat)) (mat-to-buffered-image mat))]
...
(defn render [^JFrame frame ^Graphics g]
  (let [{:keys [images selected] :as state} @appstate
        ...
        image (if (and im (pos? (.getWidth frame))) (mat-to-buffered-image mat))
...
(def target-homography
  {:9x9
   (doto (MatOfPoint2f.)
     (.fromList (for [ x (range 1 10) y (range 1 10)] (Point. (* 70.0 x) (* 70.0 y)))))
   :13x13
   (doto (MatOfPoint2f.)
     (.fromList (for [ x (range 1 14) y (range 1 14)] (Point. (* 70.0 x) (* 70.0 y)))))
   :19x19
   (doto (MatOfPoint2f.)
     (.fromList (for [ x (range 1 19) y (range 1 19)] (Point. (* 70.0 x) (* 70.0 y)))))})
...
(defn find-goban [gray-img colour]
  (let [contours (ArrayList.) hier (Mat.)
        c (--> gray-img (Imgproc/Canny 5 50 3 false))]
    ;; Find contours
  ...
  (do
    ...
    (let [fil (Mat.) m (Mat.) m2 (Mat.) edges (Mat.) hough (Mat.) hough-img (Mat.)
          ...
          (-->
            ...
            (Imgproc/bilateralFilter 15 (double 15) (double 15))
            )
          ...
          goban-contour (vec->mat (MatOfPoint2f.) goban-corners)]
      ...
      (let
        [goban-points
         ...
         (condp > (count goban-points)
           300 {:size 19 :target (:19x19 target-homography)}
           130 (:size 13 :target (:13x13 target-homography))
           {:size 9 :target (:9x9 target-homography)}
           )
         sorted
         (sort-by
           (juxt
             (comp #(int (/ % 25)) (partial line-to-point-dist [p1 pf]))
             (comp #(int (/ % 25)) (partial line-to-point-dist [p1 pl])))
           (take (.rows target) goban-points))
         origpoints
         (doto (MatOfPoint2f.)
           (.fromList (map (fn [[x y]] (Point. x y)) sorted)))
         h (if (= (.rows target) (count sorted))
             (Calib3d/findHomography origpoints target Calib3d/FM_RANSAC 3))]

        (doseq [[i [x y]] (map-indexed vector sorted)]
          (Core/circle colour (Point. x y)
            i (Scalar. 255 25 25) 2))
        ...
        (when-let [h @current-transform]
          (let [transformed (Mat.) ih (Mat.) invert-transformed (Mat.)]
            ...
            (Imgproc/erode detectimg detectimg (Imgproc/getStructuringElement Imgproc/MORPH_RECT (Size. 10 10)))
          ...
          (doseq [x (range 1 (inc size))]
            (doseq [y (range 1 (inc size))]
              (let [p (Point. (- (* 70.0 x) 15) (- (* 70.0 y) 15))
