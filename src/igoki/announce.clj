(ns igoki.announce
  ...
  (:import (kuusisto.tinysound TinySound)
           (java.io InputStream FileInputStream BufferedInputStream ByteArrayOutputStream ByteArrayInputStream)
           (sun.audio AudioStream AudioPlayer)
           (javax.sound.sampled AudioSystem DataLine$Info SourceDataLine AudioInputStream Clip LineListener LineEvent LineEvent$Type)
...
(defn announce [parts]
  ...
  (doseq [p (remove #(or (nil? %) (str/blank? %)) parts)]
    (case p
      "," (Thread/sleep 250)
      "-" (Thread/sleep 500)
      (sound (str "sounds/japanese/" p ".wav")))))
